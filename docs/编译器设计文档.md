...
### 6.1 LLVM IR 中间代码生成设计

​	本编译器主要参考LLVM IR官方文档和编译教程中的介绍实现了一个简单的LLVM IR类型系统，主要使用继承关系实现。

<img src="img/structure.png">

#### 6.1.1 ValueType

​	本编译器的类型系统中包含数组类型、函数类型、指针类型、标签类型、Int类型、Char类型、Void类型，他们均继承于一个基类ValueType(这里感谢荣文戈老师开设的C语言系统级编程在此部分设计中的帮助)。

​	数组类型中包含数组元素类型和数组大小两个字段：
...
​	这里需要注意LLVM IR系统中bool类型是一个`i1`类型，这一点我在IntType中进行标记：
...
​	LLVM IR类型中顶层模块为Module,一个Module中包含多个GlobalVariable和多个Function，在每个Function中包含多个BasicBlock,在每个BasicBlock中包含多个Instruction.由于在一个程序中只能存在一个顶层模块，这里对于Module类使用了单例模式。

```cpp
struct Module : public IRPrintable { // 单例模式
    private :
        Module() {}
        ~Module() {}
...
​	本编译器使用到的LLVM IR指令有：AllocaInstr, ArithmeticInstr(多种算数类型指令加、减、乘、除、取模), BrInstr, RetInstr, CallInstr, GetelementptrInstr, IcmpInstr, StoreInstr, LoadInstr, ZextInstr, TruncInstr.使用的数据结构建模如下：
...
​	Visitor是中间代码生成的核心类，我认为中间代码生成的难点主要在于Int和Char类型之间的转换以及If,For等控制流的生成。

##### 6.2.4.1 类型转换

​	对于类型转换的时机可以概括为“用到时候判断，保存时候判断”。所谓“用到时候判断”，例如在生成算数指令时，必须要将char类型的操作数转换为int类型的操作数(`zext`)；所谓“保存时候判断”，即为在赋值语句中对表达式左值进行赋值时进行判断。另外需要注意的点就是在函数调用过程中（传参、返回值）进行类型转换。类型转换代码如下例子：
...
​	控制流的生成我主要使用了基本块栈的方法，对于基本块编号采用编号回填的办法。
...
​	对于if和for控制流，创建多个基本块，使用br指令在基本块之间跳转，if控制流的基本块结构图如下：

<img src="img/if.png" height="300px">
...
<img src="img/for.png" height="300px">
...
​	基于LLVM IR中间代码生成mips指令难度不大，我首先实现了栈式内存管理，没有进行寄存器分配，我将实际变量(全局变量或alloca的变量)和LLVM IR中的虚拟寄存器统一保存在栈中。
...
​	对于任何的LLVM IR指令，我们至多只需要三个物理寄存器就可以实现。主要需要注意的是int类型和char类型的内存对齐问题。在全局数据段`.data`中存储的数据MARS会进行自动对齐，但在栈区需要进行手动对齐。我的解决办法是在使用alloca指令移动栈指针时，一定分配4的倍数的内存。
...
​	生成MIPS代码的思路比较直接，直接对中间代码中的LLVM IR指令每一条进行翻译即可，实现难度不大。
